...
trait NumericFunctorOps[+Self[+_], -Codomain, +Output <: Codomain]
  ...
  /** Alias for [[scale]]. */
  final inline infix def * [Number >: Output <: Codomain : Numeric as Number]
    (factor: Number)
    : Self[Number] = scale(factor)

  /** Replace each element with a version divided by [[divisor]]. */
  final inline def divide[Number >: Output <: Codomain : Fractional as Number]
    (divisor: Number)
    : Self[Number] = map(Number.div(_, divisor))

  /** Alias for [[divide]]. */
  final inline infix def / [Number >: Output <: Codomain : Fractional as Number]
    (divisor: Number)
    : Self[Number] = divide(divisor)

  /** Replace each element with a version divided by [[divisor]] (rounded down). */
  final inline def floorDivide[
    Number >: Output <: Codomain : Integral as Number,
  ](divisor: Number): Self[Number] = map(Number.quot(_, divisor))

  /** Alias for [[floorDivide]]. */
  final inline infix def :/ [Number >: Output <: Codomain : Integral as Number]
    (divisor: Number)
    : Self[Number] = floorDivide(divisor)

  /** Replace each element with a version modulo [[base]]. */
  final inline def mod[Number >: Output <: Codomain : Integral as Number]
    (base: Number)
  ...
  /** Alias for [[negate]]. */
  final inline def unary_-[Number >: Output <: Codomain : Numeric as Number]
    : Self[Number] = negate

  /** Replace each element with its absolute value. */
  final inline def abs[Number >: Output <: Codomain : Numeric as Number]
    : Self[Number] = map(Number.abs)

  /** Replace each element with its sign (`1` or `-1`). */
  final inline def sign[Number >: Output <: Codomain : Numeric as Number]
    : Self[Number] = map(Number.sign)

  /** Replace each element with its multiplicative inverse. */
  final inline def reciprocal[
    Number >: Output <: Codomain : Fractional as Number,
  ...
  /**
    ...
    * `false`.
    */
  final inline def support[Number >: Output : Numeric as Number]
