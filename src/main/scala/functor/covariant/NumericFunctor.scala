...
import io.github.sgtswagrid.nonsense.transform.NegateOps
...
trait NumericFunctor[+Self[+_], +Output : Numeric as Number]
  extends ContextFunctor[Self, Numeric, Output], NegateOps[Self[Output]]:

  final inline def negate: Self[Output] = map(Number.negate)
